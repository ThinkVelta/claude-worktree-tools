...
| `redact-secrets.jq` | PEM body validated **per line** as base64 or one of the two named encrypted-key headers, and required to contain **one base64 run of 40+ characters**; `END` label must match `BEGIN` via `\1` | An unrestricted `[\s\S]*?` matches from any `BEGIN` marker to any later `END`, so a `grep -rn 'PRIVATE KEY' .` transcript lost 21 of 24 lines between two unrelated hits. Restricting to base64 alone then dropped **encrypted** keys, whose `Proc-Type:` / `DEK-Info:` headers contain `:` `,` `-` — naming the two headers readmits them without re-opening the body to arbitrary text. A character class of any kind is still too weak: letters and spaces both belong to it, so prose between two same-labelled markers was still swallowed; validating whole lines excludes the space, and the length floor excludes single-word lines (`hello` is valid base64 alphabet). The backreference stops `BEGIN RSA` pairing with a later `END EC`. **Not** quartet-validated — `(?:[A-Za-z0-9+/]{4})*` per line looks stricter but drops real keys, since OpenSSH ed25519 bodies wrap at 70 characters and EC bodies contain 27/30/36-character lines, none of them multiples of 4. Verified against openssl- and ssh-keygen-generated RSA, encrypted RSA, EC, PKCS#8 and ed25519 keys. |
