...
| `redact-secrets.jq` | PEM handled in **two layers**: the regex decides the block's shape (matching `BEGIN`/`END` labels via `\k<lbl>`, each body line base64 or one of the two named encrypted-key headers), and the `pem_body_ok` function decides whether the payload is really base64 — concatenating the body lines and requiring legal terminal padding and a total length divisible by 4 | An unrestricted `[\s\S]*?` matches from any `BEGIN` marker to any later `END`, so a `grep -rn 'PRIVATE KEY' .` transcript lost 21 of 24 lines between two unrelated hits. Restricting to base64 alone then dropped **encrypted** keys, whose `Proc-Type:` / `DEK-Info:` headers contain `:` `,` `-`. A character class of any kind is still too weak — letters and spaces both belong to it, so prose between two same-labelled markers was swallowed; and validating whole lines still admits a body of single words, since `hello` is valid base64 alphabet. Deciding the payload is not something a regex can do, which is why it moved into jq. **Not** quartet-validated per line: that reads as stricter but drops real keys, since OpenSSH ed25519 bodies wrap at 70 characters and EC bodies contain 27/30/36-character lines, none of them multiples of 4 — concatenating is exactly what removes wrap width. **Not** DER-validated either: only 2 of the 5 key types openssl and ssh-keygen emit start with DER `0x30`, an encrypted body being raw ciphertext and an OpenSSH body starting `6f 70 65 6e` (`"open"`), so requiring DER would stop masking both. Verified against RSA, encrypted RSA, EC, PKCS#8 and ed25519. Residual: the mod-4 test cuts accidental matches ~4×, it does not eliminate them — a non-key body that happens to be all-base64 characters and a multiple of 4 long is still over-redacted. |
